...
- a. PS; b. WS; c. PS; d. Not possible to predict    (Correct)

- a. PS; b. Not possible to predict; c. PS; d. Not possible to predict

- a. PS; b. WS; c. PS; d. WS

- a. Not possible to predict; b. WS; c. PS; d. PS
...
inTrain <- createDataPartition(y = segmentationOriginal$Case, p = 0.6, 
                               list = FALSE) 
training <- segmentationOriginal[inTrain, ] # 60% training
testing <- segmentationOriginal[-inTrain, ] # 40% test
...
# 3.
# Looking at finalModel output we can predict solutions:
model$finalModel
...
- The bias is larger and the variance is smaller. Under leave one out cross validation K is equal to the sample size.   (Correct)
...
- 2.783. It is strange because Area should be a qualitative variable - but tree is reporting the average value of Area as a numeric variable in the leaf predicted for newdata.   (Correct)
...
# Create model with Area as outcome variable and default arguments in tree command:
...
- Test Set Misclassification: 0.31; Training Set: 0.27    (Correct)
...
I am using different version of packages. Solution should be the only one with Test set equal to 0.31.
...
- The order of the variables is: x.2, x.1, x.5, x.6, x.8, x.4, x.9, x.3, x.7,x.10   (Correct)
...
Variables 2 and 1 are changed because of data package version.